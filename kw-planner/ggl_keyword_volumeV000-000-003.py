...
# File version: V000-000-003
# Note: Imports the results into a csv, filtered. Combines v000-000-001 & v000-000-002
# insert into prompt: python c:\MyPrograms\workstation\GoogleAdsSearchReportsHrly\keywordplanner\ggl_keyword_volumeV000-000-003.py -c 5428037747 
...
_DEFAULT_KEYWORD_TEXT = ["nvda","tsla"]
...
def main(
    client, customer_id, location_ids, language_id, keyword_texts, page_url, output_file
):
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['Keyword', 'Average Monthly Searches', 'Competition']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()
        exact_match_keywords = set(keyword_texts)
        for idea in keyword_ideas:
            if idea.text in exact_match_keywords:
    #            for idea in keyword_ideas:
                    writer.writerow({
                        'Keyword': idea.text,
...
if __name__ == "__main__":
    ...
    try:
        main(
            ...
            args.keyword_texts,
            args.page_url,
            output_file = 'keywordplanner/output/kwplanner_tickers.csv',  # Output CSV file
        )
