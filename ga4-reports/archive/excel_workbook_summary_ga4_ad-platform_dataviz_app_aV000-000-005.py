...
# Notes: Creates Chart visualizations on the pivot_sheet - builds the 2nd column & 3rd of charts, maps report view metrics to data source metric names
...
Sub CreatePerformanceCharts()
    ...
    For i = LBound(landlordMetrics) To UBound(landlordMetrics)
        ...
        chartTopPosition = ws.Cells(13 + (i * 22), 5).Top  ' Column E, move down for each chart iteration

        ' Create the chart
        Set chart = ws.ChartObjects.Add(Left:=chartLeftPositionLandlord, Top:=chartTopPosition, Width:=600, Height:=300)  ' Adjust position and size
        ...
        ' Insert chart prefix concatenation formula in the row directly above the chart (Row 12)
        chartPrefix = "=R3 & "" - "" & " & ws.Cells(7 + 1, rangeStartCol).Address(False, False)
        ws.Cells(12 + (i * 22), 5).Formula = chartPrefix

        ' Set the chart title to reference the concatenation result in row 12
        chart.Chart.HasTitle = True
        chart.Chart.ChartTitle.Formula = "=" & ws.Cells(12 + (i * 22), 5).Address(External:=True)
    ...
    For i = LBound(travelerMetrics) To UBound(travelerMetrics)
        ...
        ' Define the X-axis range (first column) and series ranges (second, third, and fourth columns)
        Set chartXValues = ws.Range(ws.Cells(startRow + 1, rangeStartCol), ws.Cells(startRow + 1 + numRows - 1, rangeStartCol))
        ...
        ' Insert section name "Traveler Performance" only for the first chart in row 10, column Q
        If firstTravelerChart Then
            ws.Cells(10, 19).Value = "Traveler Performance"
            ws.Cells(10, 19).Font.Size = 20
            firstTravelerChart = False
        End If

        ' Adjust the chart position dynamically based on the iteration
        chartTopPosition = ws.Cells(13 + (i * 22), 17).Top  ' Column Q for traveler charts

        ' Create the chart
        Set chart = ws.ChartObjects.Add(Left:=chartLeftPositionTraveler, Top:=chartTopPosition, Width:=600, Height:=300)
        chart.Chart.SetSourceData Source:=chartYValuesPrimary
        ...
        chartPrefix = "=R3 & "" - "" & " & ws.Cells(7 + 1, rangeStartCol).Address(False, False)
        ws.Cells(12 + (i * 22), 19).Formula = chartPrefix

        ' Set the chart title to reference the concatenation result in row 12
        chart.Chart.HasTitle = True
        chart.Chart.ChartTitle.Formula = "=" & ws.Cells(12 + (i * 22), 19).Address(External:=True)
    ...
    For i = LBound(generalMetrics) To UBound(generalMetrics)
        ...
        ' Insert section name "General Performance" only for the first chart in row 10, column AG
        If firstGeneralChart Then
            ws.Cells(10, 33).Value = "General Performance"
            ws.Cells(10, 33).Font.Size = 20
            firstGeneralChart = False
        End If

        ' Adjust the chart position dynamically based on the iteration
        chartTopPosition = ws.Cells(13 + (i * 22), 17).Top  ' Column AG for general charts

        ' Create the chart
        Set chart = ws.ChartObjects.Add(Left:=chartLeftPositionGeneral, Top:=chartTopPosition, Width:=600, Height:=300)
        chart.Chart.SetSourceData Source:=chartYValuesPrimary
        ...
        ' Insert chart prefix concatenation formula in the row directly above the chart
        chartPrefix = "=R3 & "" - "" & " & ws.Cells(7 + 1, rangeStartCol).Address(False, False)
        ws.Cells(12 + (i * 22), 33).Formula = chartPrefix

        ' Set the chart title to reference the concatenation result in row 12
        chart.Chart.HasTitle = True
        chart.Chart.ChartTitle.Formula = "=" & ws.Cells(12 + (i * 22), 33).Address(External:=True)
