...
def fetch_responsive_search_ad_asset_performance(client, customer_id, messaging_category_map, ad_performance_map):
    ...
    try:
        ...
        for batch in asset_response:
            for row in batch.results:
                ...
                # Lookup corresponding ad performance data
                ad_data = ad_performance_map.get((campaign_id, ad_group_id, ad_id, date), {})
                ad_impressions = ad_data.get('impressions', 0)
                ad_clicks = ad_data.get('clicks', 0)
                ad_cost_micros = ad_data.get('cost_micros', 0.0)
                ad_conversions = ad_data.get('conversions', 0)

                # Calculate the ratio of asset impressions to ad impressions
                impression_ratio = (row.metrics.impressions / ad_impressions) if ad_impressions > 0 else None
                ...
                asset_results.append({
                    ...
                    'ad_id': ad_id,
                    'pinned_field': row.ad_group_ad_asset_view.pinned_field,
                    'date': date,
                    'asset_id': row.ad_group_ad_asset_view.asset,
                    'field_type': field_type,
                    'asset_type': field_type,
                    'asset_text': corrected_text,
                    'messaging_category': messaging_category,
                    'keyword_based': keyword_based,
                    'impressions': row.metrics.impressions,
                    'clicks': row.metrics.clicks,
                    'cost_micros': row.metrics.cost_micros / 1_000_000,
                    'ad_impressions': ad_impressions,
                    'ad_clicks': ad_clicks,
                    'ad_cost_micros': ad_cost_micros,
        ...
        for row in reader:
            ...
            try:
                ad_performance_map[key] = {
                    ...
                    'conversions': int(row.get('conversions', 0))  # Handle missing or non-integer values
                }
            except ValueError:
                ad_performance_map[key] = {
                    'impressions': int(row.get('impressions', 0)),
                    'clicks': int(row.get('clicks', 0)),
                    'cost_micros': float(row.get('cost_micros', 0.0)),
                    'conversions': 0  # Default to 0 if conversion value is not valid
                }
...
if __name__ == "__main__":
    ...
    # Load the ad performance data
    ad_performance_map = load_ad_performance(os.path.join(output_directory, "ad_performance.csv"))

    for customer_id in CUSTOMER_IDS:
        # Fetch performance data
        asset_performance = fetch_responsive_search_ad_asset_performance(client, customer_id, messaging_category_map, ad_performance_map)
    ...
    if all_asset_results:
        save_to_csv(all_asset_results, "asset_performance_enhanced.csv")
