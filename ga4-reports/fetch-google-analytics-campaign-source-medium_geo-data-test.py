...
def fetch_data_for_date(date_str):
    ...
    date_ranges=[DateRange(start_date="2023-07-01", end_date="2023-07-02")],
    dimensions=[
        Dimension(name="date"),
        Dimension(name="City"),
        Dimension(name="Region"),
...
while current_date <= end_date:
    ...
    pivot_df = df.pivot_table(
        index=["Date", "Source", "Medium", "Campaign", "Device", "City", "State", "Country"],
        columns="Event Name",
        values="Event Count",
        aggfunc="sum",
        fill_value=0
    ...
    for index, row in pivot_df.iterrows():
        cursor.execute("""
        INSERT INTO ga4_events_geo_data (Date, Source, Medium, Campaign, Device, City, State, Country, FF_Purchase_Event_Count, FF_Lead_Event_Count, FF_BRSubmit_Event_Count, FF_DMSubmit_Event_Count, FF_PhoneGet_Event_Count, FF_HRSubmit_Event_Count, HR_Submit_Event_New_Traveler_Lead_Count)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
