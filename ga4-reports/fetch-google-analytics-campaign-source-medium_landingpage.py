...
cursor.execute("DROP TABLE IF EXISTS ga4_events_landingpage")

# Create table with the new schema
cursor.execute("""
CREATE TABLE ga4_events_landingpage (
    Date TEXT,
    Source TEXT,
    Medium TEXT,
    Campaign TEXT,
    ...
    PRIMARY KEY (Date, Source, Medium, Campaign, Device, LandingPage)
)
""")
...
dimensions = [
    ...
    Dimension(name="landingPage"),  # New dimension for landing page
]
...
def fetch_data_for_date(date_str):
    ...
    for row in response.rows:
        data.append({
            ...
            "LandingPage": row.dimension_values[6].value,  # Capture landing page data
...
while current_date <= end_date:
    ...
    pivot_df = df.pivot_table(
        index=["Date", "Source", "Medium", "Campaign", "Device", "LandingPage"],
        columns="Event Name",
        values="Event Count",
        aggfunc="sum",
        fill_value=0
    ...
    for index, row in pivot_df.iterrows():
        cursor.execute("""
        INSERT INTO ga4_events_landingpage (Date, Source, Medium, Campaign, Device, LandingPage, FF_Purchase_Event_Count, FF_Lead_Event_Count, FF_BRSubmit_Event_Count, FF_DMSubmit_Event_Count, FF_PhoneGet_Event_Count, FF_HRSubmit_Event_Count, HR_Submit_Event_New_Traveler_Lead_Count)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(Date, Source, Medium, Campaign, Device, LandingPage) DO UPDATE SET
        ...
        """, (
            ...
            row["Campaign"],
            row["Device"],
            row["LandingPage"],  # Include landing page in the insertion statement
