...
# version: V000-000-011
# execution method: python googleads_daily_report_last52weeks.py
# Note: Builds on V000-000-010 to now loop through snapshots of the last 14 days
#       modifies 'calculate_sumary_metrics' to accept a date parameter, main script to loop through the last 14 days
...
def save_combined_to_excel(data_dict, filename):
    ...
    for date, summaries in data_dict.items():
...
if clicks_impressions_spend_data is not None:
    ...
    date_range = [datetime.today() - timedelta(days=x) for x in range(1, 14)]
    all_summaries = {}
...
# Save all summaries to Excel
    save_combined_to_excel(all_summaries, 'reports/output/summary_metrics_combined_14_days.xlsx')
    print("Data successfully saved to 'summary_metrics_combined_14_days.xlsx'")
else:
