...
# output:
# Notes: Reformats the metric column labels and inserts the core metric into the row above by creating new row copied metrics and then parsed
...
for col in ws.iter_cols(min_col=1, max_col=ws.max_column, min_row=6, max_row=ws.max_row):
    header = col[0].value
    for cell in col[1:]:
        if header in ['Actual', 'LW', 'WoW', 'YoY']:
            if any(prefix in header for prefix in ['Spend', 'Traveler Value', 'Landlord Value']):
                cell.style = dollar_no_decimal
            elif any(prefix in header for prefix in ['CPL', 'CAC']):
                cell.style = dollar_no_decimal
            elif any(prefix in header for prefix in ['CPTA']):
                cell.style = dollar_two_decimal
            elif any(prefix in header for prefix in ['ROAS']):
                cell.style = percent_no_decimal
            elif any(prefix in header for prefix in ['Lead Conversion', 'Traveler Conversion']):
