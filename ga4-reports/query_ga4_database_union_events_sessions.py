...
# output: table; csv
# Notes: Creates an exception for channel_group naming for unmatched campaign names on facebook
...
# Save and inspect Google Ads aggregated data
google_ads_debug_path = "ga4-reports/output/debug/debug_google_ads_agg.csv"
df_google_ads_agg.to_csv(google_ads_debug_path, index=False)
print(f"Google Ads aggregated data saved to {google_ads_debug_path}")

# INSPECT SHAPES - ensure no duplicate rows
print("Debugging before concatenation:")
print("GA4 combined shape:", df_combined.shape)
print("Google Ads aggregated shape:", df_google_ads_agg.shape)
print("Bing Ads shape:", df_bing_ads.shape)
print("Facebook Ads shape:", df_facebook_ads.shape)

# Inspect unique combinations for Google Ads data
print("Google Ads unique combinations:")
print(df_google_ads_agg[['Source', 'Medium', 'Campaign']].drop_duplicates())
...
print("Final combined DataFrame shape:", df_final_combined.shape)

# Check for potential duplicates
duplicates_google = df_final_combined[df_final_combined.duplicated(
    subset=['Date', 'Source', 'Medium', 'Campaign', 'Device'], keep=False)]
print(f"Potential duplicates in final combined DataFrame: {len(duplicates_google)}")
if not duplicates_google.empty:
    duplicates_google.to_csv("ga4-reports/output/debug/debug_duplicates_google.csv", index=False)
    print("Duplicates saved to ga4-reports/output/debug/debug_duplicates_google.csv")

# Save and inspect Google Ads aggregated data
df_fina_combined_debug_path = "ga4-reports/output/debug/debug_df_final_combined_pre-channelgroup.csv"
df_final_combined.to_csv(df_fina_combined_debug_path, index=False)
print(f"Google Ads aggregated data saved to {df_fina_combined_debug_path}")
...
# Define the specific conditions for "Paid Social + Display"
paid_social_display_conditions = (
    (df_final_combined['Source'] == 'facebook') &
    (df_final_combined['Medium'] == 'cpc') &
    (df_final_combined['Campaign'] == 'LL Retargeting - Conversions')
)

# Assign "Paid Social + Display" to rows matching the conditions
df_final_combined.loc[paid_social_display_conditions, 'Channel_Group'] = 'Paid Social + Display'
...
# Debugging for "Paid Social + Display" rows
paid_social_display_debug = df_final_combined[
    (df_final_combined['Channel_Group'] == 'Paid Social + Display') &
    (df_final_combined['Source'] == 'google')
]
paid_social_display_debug.to_csv("ga4-reports/output/debug/debug_debug_paid_social_display.csv", index=False)
print("Paid Social + Display rows saved to ga4-reports/output/debug/debug_debug_paid_social_display.csv")
...
# Convert all values in the "Device" column to lowercase
df_final_combined['Device'] = df_final_combined['Device'].str.lower()
