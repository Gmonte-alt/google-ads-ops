...
# Notes: This is the first version of the new Channel Grouping logic for a clean view of performance based on campaign, source, medium.
...
def assign_new_channel_group(df):
    ...
    email_df = email_df[((email_df['Source'] == 'email') & (email_df['Medium'] == 'marketing')) |
    ...
    # Filter for Display channel group
    display_df = df[df['Default_Channel_Group'] == 'Display']
    print(f"Display Channel Group - Initial Rows: {len(display_df)}")
    ...
    print(f"Display Channel Group - Filtered Rows: {len(display_df)}")
    ...
    referral_df = referral_df[(referral_df['Medium'] == 'referral') & (referral_df['Source'] != 'travelnursehousing.com')]
    ...
    paid_search_non_brand_df = paid_search_non_brand_df[(paid_search_non_brand_df['Source'].isin(['google', 'bing'])) &
                                                        ...
                                                        (~paid_search_non_brand_df['Campaign'].str.contains('display|organic|not set|prospecting|retargeting|awareness|brand|tenant|(referral)|(direct)', case=False))]
    ...
    paid_search_non_brand_tenant_df = paid_search_non_brand_tenant_df[(paid_search_non_brand_tenant_df['Source'].isin(['google', 'bing'])) &
                                                                      ...
                                                                      (paid_search_non_brand_tenant_df['Campaign'].str.contains('tenant', case=False))]
...
cleaned_df = assign_new_channel_group(df)

# Save the cleaned DataFrame to a new table in the database
cleaned_df.to_sql('ga4_channel_groups_cleaned', conn, if_exists='replace', index=False)

# Save the cleaned DataFrame to a CSV file for review
cleaned_df.to_csv('ga4-reports/output/ga4_channel_groups_cleaned.csv', index=False)
