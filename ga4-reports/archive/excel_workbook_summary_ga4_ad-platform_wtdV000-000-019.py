...
# Notes: Creates the "--DATA-WTD--" worksheet into the final excel workbook, filtering to one metric
...
def add_wtd_data_tab_to_workbook(workbook, df_wtd_adjusted):
    ...
    # Create column headers for ISO Weeks
    unique_iso_weeks = df_wtd_adjusted['ISO_Week'].unique()
    header = ['Year', 'Channel_Group', 'Campaign_Group'] + [f'ISO_Week_{week}' for week in unique_iso_weeks]
    ...
    for _, group in df_wtd_adjusted.groupby(['ISO_Year', 'Channel_Group', 'Campaign_Group']):
        row_data = [group['ISO_Year'].iloc[0], group['Channel_Group'].iloc[0], group['Campaign_Group'].iloc[0]]
        
        # Add metric values for each ISO week
        for week in unique_iso_weeks:
            value = group[group['ISO_Week'] == week]['FF_Purchase_Event_Count'].sum()  # Example metric: 'FF_Purchase_Event_Count'
