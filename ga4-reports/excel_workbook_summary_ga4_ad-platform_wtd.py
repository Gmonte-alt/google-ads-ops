...
# Notes: Incorporates new worksheet "--DATA--EXP--" and defines logic for new columns
...
# ----------------------------------------------------------------------------------- #
# ----------------- Create New Worksheet "--DATA--EXP--" for new import ------------- #
# ----------------------------------------------------------------------------------- #

# Logic for new columns
def apply_ad_platform(campaign_group):
    if any(platform in campaign_group for platform in ['Google', 'Bing', 'Facebook']) and "Total" not in campaign_group:
        return next(platform for platform in ['Google', 'Bing', 'Facebook'] if platform in campaign_group)
    return "(none)"

def apply_paid_or_nonpaid(channel_group):
    return "Paid" if any(keyword in channel_group for keyword in ['SEM', 'Paid', 'Display']) else "Non-Paid"

def apply_brand_or_nonbrand(channel_group):
    # return "Brand" if channel_group == 'SEM Brand' else "Non-Brand" # changed the definition of the Brand or Non-Brand slicer to include Direct
    if channel_group == 'SEM Brand' or 'Direct' in channel_group:
        return "Brand"
    return "Non-Brand"

def apply_total_camp_group(campaign_group):
    return "Total" if "Total" in campaign_group else campaign_group

def apply_total(campaign_group):
    return "Total" if "Total" in campaign_group else "Non-Total"

# New logic for "Paid Media Type"
def apply_paid_media_type(channel_group):
    if 'SEM' in channel_group:
        return 'SEM'
    elif 'Paid Social + Display' in channel_group:
        return 'Paid Social + Display'
    else:
        return 'Non-Paid'

# New logic for "Tenant or Landlord"
def apply_tenant_or_landlord(campaign_group):
    if any(keyword in campaign_group for keyword in ['Tenant', 'Traveler']):
        return 'Tenant'
    else:
        return 'Landlord'

def add_exp_data_tab_to_workbook(workbook, csv_path):
    # Check if '--DATA--EXP--' sheet already exists and remove it if necessary
    if '--DATA--EXP--' in workbook.sheetnames:
        del workbook['--DATA--EXP--']
    
    # Create a new worksheet for the EXP data
    ws = workbook.create_sheet(title='--DATA--EXP--')

    # Load the CSV data into a DataFrame
    df_exp = pd.read_csv(csv_path)

    # Add new columns based on the logic
    df_exp['Ad Platform'] = df_exp['Campaign_Group'].apply(apply_ad_platform)
    df_exp['Paid or Non-Paid'] = df_exp['Channel_Group'].apply(apply_paid_or_nonpaid)
    df_exp['Brand or Non-Brand'] = df_exp['Channel_Group'].apply(apply_brand_or_nonbrand)
    df_exp['Paid Media Type'] = df_exp['Channel_Group'].apply(apply_paid_media_type)  # New column
    df_exp['Tenant or Landlord'] = df_exp['Campaign_Group'].apply(apply_tenant_or_landlord)  # New column
    df_exp['Campaign_Group'] = df_exp['Campaign_Group'].apply(apply_total_camp_group)
    df_exp['Total'] = df_exp['Campaign_Group'].apply(apply_total)

    # Reorder columns: new columns first, followed by existing ones
    new_columns_order = ['Ad Platform', 'Paid or Non-Paid', 'Brand or Non-Brand', 'Paid Media Type', 'Tenant or Landlord', 
                         'Period', 'ISO_Year', 'ISO_Week', 'Total', 'Channel_Group', 'Campaign_Group']
    remaining_columns = [col for col in df_exp.columns if col not in new_columns_order]
    df_exp = df_exp[new_columns_order + remaining_columns]

    # Insert the DataFrame into the worksheet
    for r in dataframe_to_rows(df_exp, index=False, header=True):
        ws.append(r)

    # Optionally, you can adjust column widths or add formatting here

    # Save the workbook after adding the sheet
    workbook.save(excel_file_path)

    print(f"Worksheet '--DATA--EXP--' has been added to {excel_file_path}.")

# Usage
exp_csv_path = 'ga4-reports/output/ga4_wtd_fullweek_adjusted_data.csv'
add_exp_data_tab_to_workbook(workbook, exp_csv_path)
