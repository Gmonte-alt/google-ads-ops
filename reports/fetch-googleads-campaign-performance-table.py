# file name: reports/fetch-googleads-campaign-performance-table.py
# version: V000-000-002
# output:
# Notes: Adjusting the database to delete the last 60 days and then back-fill from that date through yesterday
...
# Calculate the cut-off date for the last 60 days
cutoff_date = (datetime.today() - timedelta(days=60)).strftime('%Y-%m-%d')
...
if table_exists:
    # Delete the last 60 days of data
    cursor.execute("""
    DELETE FROM google_ads_campaign_performance WHERE Date >= ?;
    """, (cutoff_date,))
    conn.commit()
    print(f"Deleted data from {cutoff_date} onwards.")
    
    # Check the last date of data in the table after deletion
    cursor.execute("""
    ...
    # If the table does not exist, start from January 1, 2023
...
if start_date:
    ...
    # Create table if it doesn't exist or insert new data if it does
    cursor.execute("""
