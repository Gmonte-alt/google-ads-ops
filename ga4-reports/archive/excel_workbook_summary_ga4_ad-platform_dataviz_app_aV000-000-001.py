...
# Notes: Creates Chart Prefix to be used in the next iteration of the report for Chart labeling
...
Sub CreateFilterInformation()
    ...
    ' Reference the first pivot table slicer values (modify as necessary to reference correct slicer values)
    For i = LBound(slicerNames) To UBound(slicerNames)
        ' Here we reference the first pivot table for the slicers (adjust reference as needed)
        ws.Cells(filteredValuesRowStart, slicerStartCol + i).Formula = _
        "=GETPIVOTDATA(" & CHR(34) & "Slicer_" & slicerNames(i) & CHR(34) & "," & CHR(34) & "PivotTable_FF_Purchase_Event_Count" & CHR(34) & ")"
    Next i
...
def insert_columns_and_add_slicers():
    ...
    try:
        ...
        slicer_left_position = 200  # Adjust left position for top slicers
