...
# Load the updated table with UTM dimensions
query = '''SELECT * FROM ga4_channel_groups_all_utms;'''
...
full_cleaned_df.to_sql('ga4_channel_groups_all_utms_cleaned', conn, if_exists='replace', index=False)

# Save the cleaned DataFrame to a CSV file for review
full_cleaned_df.to_csv('ga4-reports/output/ga4_channel_groups_all-utms_cleaned.csv', index=False)
