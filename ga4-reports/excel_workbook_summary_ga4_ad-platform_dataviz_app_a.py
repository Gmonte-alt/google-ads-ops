...
# version: V000-000-010
# Notes: Modifies the pivot table and range metrics to incorporate new metrics for Traveler & Landlord Revenue, and all derived metrics

### Enhanced Python Script

## Now, I’ll provide the complete Python script that integrates with the updated VBA code chart creation that modifies the Primary y-axis for each mapped metric
...
Sub CreatePivotTableWithSharedCache(metricName As String, pivotTableName As String, topPosition As Double, leftPosition As Double)
    ...
    ' Check if the sheet "--CHART--WEEKLY--" exists, and create it if it doesn't
    On Error Resume Next
    Set wsPivot = ThisWorkbook.Sheets("--CHART--WEEKLY--")
    On Error GoTo 0
    If wsPivot Is Nothing Then
        Set wsPivot = ThisWorkbook.Sheets.Add(After:=ThisWorkbook.Sheets(ThisWorkbook.Sheets.Count))
        wsPivot.Name = "--CHART--WEEKLY--"
    End If
    ...
    pivotTable.PivotFields("Device type").Orientation = xlPageField  ' Device type filter Field
    pivotTable.PivotFields("Form factor").Orientation = xlPageField  ' Form factor filter Field
...
Sub CreateReferenceRangeAndYoY(pivotTableName As String, metricName As String, rangeStartRow As Integer, rangeStartColumn As Integer)
    ...
    For i = 1 To 52
        ...
        wsPivot.Cells(currentRow, rangeStartColumn + 3).Formula = "=IF(" & wsPivot.Cells(currentRow, rangeStartColumn + 1).Address & "=0," & Chr(34) & Chr(34) & "," & _
                                                                    "IFERROR((" & wsPivot.Cells(currentRow, rangeStartColumn + 1).Address & "/" & _
                                                                    wsPivot.Cells(currentRow, rangeStartColumn + 2).Address & ")-1,0))"
...
Sub CreateFilterInformation()
    ...
    Set ws = ThisWorkbook.Sheets("--CHART--WEEKLY--")
    ...
    slicerNames = Array("Paid or Non-Paid", "Device type", "Form factor", "Tenant or Landlord", "Paid Media Type", "Brand or Non-Brand", "Ad Platform", "Channel_Group", "Campaign_Group", "Period")
    numFilters = UBound(slicerNames) - LBound(slicerNames) + 1  ' Total number of slicers (10 in this case)
    ...
    ws.Cells(chartPrefixRowStart, slicerStartCol).Formula = _
        "=TEXTJOIN("" - "",TRUE,R" & filteredValuesRowStart & ":AA" & filteredValuesRowStart & ")"
    
    ' Format font color and italicize for Q1:Y3
    With ws.Range("Q1:AA3").Font
    ...
    ws.Hyperlinks.Add Anchor:=ws.Cells(5, firstCol), Address:="", SubAddress:="'Read_me'!B113", TextToDisplay:="Read me: How to use this report."
...
Sub CreatePerformanceCharts()
    ...
    Dim metricFormatMap As Object

    ' Define formatting rules
    Set metricFormatMap = CreateObject("Scripting.Dictionary")
    
    chartIndex = 1  ' Start indexing from 1
    ...
    ' Fill the range A1:AY300 with white
    With ws.Range("A1:AY300")
    ...
    landlordMetrics = Array("FF_Purchase_Event_Count", "Cost / FF_Purchase_Event_Count", "FF_Purchase_Event_Count / Sessions", "FF_Purchase_Event_Count / Clicks", _ 
                            "FF_Lead_Event_Count", "Cost / FF_Lead_Event_Count", "FF_Lead_Event_Count / Sessions", "FF_Lead_Event_Count / Clicks", "FF_Purchase_Event_Count / FF_Lead_Event_Count", "Landlord Value", "Landlord Value / Clicks")
    travelerMetrics = Array("Total Traveler Actions", "Cost / Total Traveler Actions", "Total Traveler Actions / Sessions", "Total Traveler Actions / Clicks", _ 
                            "FF_BRSubmit_Event_Count", "Cost / FF_BRSubmit_Event_Count", "FF_BRSubmit_Event_Count / Sessions", "FF_BRSubmit_Event_Count / Clicks", "FF_BRSubmit_Event_Count / Total Traveler Actions", "Traveler Value", "Traveler Value / Clicks")
    generalMetrics = Array("Impressions", "Cost", "Sessions", "Clicks", "Impressions / Impression_population", "Cost / Clicks", "Cost / Sessions", "Clicks / Impressions", "Sessions / Clicks", "Total Revenue", "Total Revenue / Cost")
    
    ' Add Landlord metrics formatting
    metricFormatMap.Add "FF_Purchase_Event_Count", "#,##0"
    metricFormatMap.Add "Cost / FF_Purchase_Event_Count", "$#,##0"
    metricFormatMap.Add "FF_Purchase_Event_Count / Sessions", "0.0%"
    metricFormatMap.Add "FF_Purchase_Event_Count / Clicks", "0.0%"
    metricFormatMap.Add "FF_Lead_Event_Count", "#,##0"
    metricFormatMap.Add "Cost / FF_Lead_Event_Count", "$#,##0"
    metricFormatMap.Add "FF_Lead_Event_Count / Sessions", "0.0%"
    metricFormatMap.Add "FF_Lead_Event_Count / Clicks", "0%"
    metricFormatMap.Add "FF_Purchase_Event_Count / FF_Lead_Event_Count", "0%"
    metricFormatMap.Add "Landlord Value", "$#,##0"
    metricFormatMap.Add "Landlord Value / Clicks", "$#,##0.00"

    ' Add Traveler metrics formatting
    metricFormatMap.Add "Total Traveler Actions", "#,##0"
    metricFormatMap.Add "Cost / Total Traveler Actions", "$#,##0"
    metricFormatMap.Add "Total Traveler Actions / Sessions", "0%"
    metricFormatMap.Add "Total Traveler Actions / Clicks", "0%"
    metricFormatMap.Add "FF_BRSubmit_Event_Count", "#,##0"
    metricFormatMap.Add "Cost / FF_BRSubmit_Event_Count", "$#,##0"
    metricFormatMap.Add "FF_BRSubmit_Event_Count / Sessions", "0.0%"
    metricFormatMap.Add "FF_BRSubmit_Event_Count / Clicks", "0%"
    metricFormatMap.Add "FF_BRSubmit_Event_Count / Total Traveler Actions", "0%"
    metricFormatMap.Add "Traveler Value", "$#,##0"
    metricFormatMap.Add "Traveler Value / Clicks", "$#,##0.00"

    ' Add General metrics formatting
    metricFormatMap.Add "Impressions", "#,##0"
    metricFormatMap.Add "Cost", "$#,##0"
    metricFormatMap.Add "Sessions", "#,##0"
    metricFormatMap.Add "Clicks", "#,##0"
    metricFormatMap.Add "Impressions / Impression_population", "0%"
    metricFormatMap.Add "Cost / Clicks", "$#,##0.00"
    metricFormatMap.Add "Cost / Sessions", "$#,##0.00"
    metricFormatMap.Add "Clicks / Impressions", "0%"
    metricFormatMap.Add "Total Revenue", "$#,##0"
    metricFormatMap.Add "Sessions / Clicks", "0%" 
    metricFormatMap.Add "Total Revenue / Cost", "0%"
    ...
    ' Consistent width and height for each chart
    Const CHART_WIDTH As Double = 623.52
    Const CHART_HEIGHT As Double = 293
    
    Application.ScreenUpdating = False  ' Turn off screen updating for smoother processing
    ...
    For i = LBound(landlordMetrics) To UBound(landlordMetrics)
        ...
        Set chartXValues = ws.Range(ws.Cells(startRow + 1, rangeStartCol - 1), ws.Cells(startRow + 1 + numRows - 1, rangeStartCol - 1))  ' X-axis range (first column)
        ...
        ' Create the chart
        Set chart = ws.ChartObjects.Add(Left:=10, Top:=10, Width:=CHART_WIDTH, Height:=CHART_HEIGHT)  ' Adjust position and size
        chart.Chart.SetSourceData Source:=chartYValuesPrimary
        chartIndex = chartIndex + 1
        
        ' Set the y-axis format for primary y-axis based on metric
        If metricFormatMap.exists(metric) Then
            chart.Chart.Axes(xlValue, xlPrimary).TickLabels.NumberFormat = metricFormatMap(metric)
        End If
   ...
   chartIndex = 1
    ...
    ' Set constants for standard position adjustment and rounding
    Const CHART_LEFT_POS As Double = 10  ' This should be the default left offset value you need
    Const CHART_TOP_OFFSET As Double = 22  ' Adjust this to the spacing you need between each chart row

    ' Reposition Landlord Charts
    For i = LBound(landlordMetrics) To UBound(landlordMetrics)
        With ws.ChartObjects(chartIndex)
            .Top = Round(ws.Cells(19 + CLng(i * CHART_TOP_OFFSET), 5).Top, 0)
            .Left = Round(ws.Cells(19 + CLng(i * CHART_TOP_OFFSET), 5).Left, 0)
    ...
    ' Reposition Traveler Charts
    For i = LBound(travelerMetrics) To UBound(travelerMetrics)
        With ws.ChartObjects(chartIndex)
            .Top = Round(ws.Cells(19 + CLng(i * CHART_TOP_OFFSET), 19).Top, 0)
            .Left = Round(ws.Cells(19 + CLng(i * CHART_TOP_OFFSET), 19).Left, 0)
            .Placement = xlMoveAndSize
            .Width = CHART_WIDTH
            .Height = CHART_HEIGHT
        End With
    ...
    ' Reposition General Charts
    For i = LBound(generalMetrics) To UBound(generalMetrics)
        With ws.ChartObjects(chartIndex)
            .Top = Round(ws.Cells(19 + CLng(i * CHART_TOP_OFFSET), 33).Top, 0)
            .Left = Round(ws.Cells(19 + CLng(i * CHART_TOP_OFFSET), 33).Left, 0)
            .Placement = xlMoveAndSize
            .Width = CHART_WIDTH
            .Height = CHART_HEIGHT
        End With
...
metrics = ["FF_Purchase_Event_Count", "Landlord Value", "FF_Lead_Event_Count", "Total Traveler Actions", "Traveler Value", "Total Revenue", "FF_BRSubmit_Event_Count", "Cost", "Sessions", "Clicks", "Impressions", "Impression_population"]  # Update with your metrics
...
metrics = ["FF_Purchase_Event_Count", "Landlord Value", "FF_Lead_Event_Count", "Total Traveler Actions", "Traveler Value", "Total Revenue", "FF_BRSubmit_Event_Count", "Cost", "Sessions", "Clicks", "Impressions", "Impression_population"]
...
metrics = ["FF_Purchase_Event_Count", "Landlord Value", "FF_Lead_Event_Count", "Total Traveler Actions", "Traveler Value", "Total Revenue", "FF_BRSubmit_Event_Count", "Cost", "Sessions", "Clicks", "Impressions", "Impression_population"]  # Update with your metrics FF_BRSubmit_Event_Count
range_start_row = 12  # Starting row for the reference range # Changed from default value = 30
range_start_column = 66  # Starting column for the reference range (adjust as needed) changed from 56 to 60
...
try:
    for metric in metrics:
        ...
        wsRange = final_workbook.Sheets("--CHART--WEEKLY--")
...
derived_metrics = ["FF_Purchase_Event_Count / FF_Lead_Event_Count", "Cost / FF_Purchase_Event_Count", "Cost / Total Traveler Actions", 
                   ...
                   "Impressions / Impression_population", "Cost / Clicks", "Cost / Sessions", "Clicks / Impressions", "Landlord Value / Clicks", "Traveler Value / Clicks",
                   "Sessions / Clicks", "Total Revenue / Cost"] # 
derived_range_start_row = 12  # Same start row as primary metric ranges
derived_range_start_column = 135  # Adjust the starting column for derived metrics # changed from 102 to 125
...
ws = final_workbook.Sheets("--CHART--WEEKLY--")
...
def insert_columns_and_add_slicers():
    ...
    try:
        ...
        left_slicer_fields = ["Paid or Non-Paid", "Device type", "Form factor", "Paid Media Type", "Brand or Non-Brand", "Ad Platform", "Channel_Group", "Campaign_Group"]

        # Slicers to be positioned horizontally on top
        top_slicer_fields = ["Tenant or Landlord", "Period", "ISO_Year"] # ["Tenant or Landlord", "Total", "Period", "ISO_Year"]
...
# "FF_Purchase_Event_Count", "Landlord Value", "FF_Lead_Event_Count", "Total Traveler Actions", "Traveler Value", "Total Revenue", "FF_BRSubmit_Event_Count", "Cost", "Sessions", "Clicks", "Impressions", "Impression_population"
# Mapping between data source metric names and ReportViewMetricName
metric_mapping = {
    "FF_Purchase_Event_Count": "New Properties",
    "Landlord Value": "Landlord Value",
    "FF_Lead_Event_Count": "Leads",
    "Total Traveler Actions": "Total Traveler Actions",
    "Traveler Value": "Traveler Value",
    ...
    "Landlord Value / Clicks": "RPC - LL",
    "Traveler Value / Clicks": "RPC - TRV",
    "Total Revenue": "Total Revenue",
    "Sessions / Clicks": "Sessions / Clicks", 
    "Total Revenue / Cost": "ROAS"
}
