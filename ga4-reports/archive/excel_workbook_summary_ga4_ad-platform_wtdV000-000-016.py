...
# output: ga4-reports/input/xChannelGrowthMarketing_Read_Me.xlsx 'ga4-reports/input/Picture1.png
# Notes: Builds on V000-000-015, inserts a title worksheet & read_me workshet from an existing excel workbook and formats the worksheets
...
def insert_commentary(worksheet_name):
    ...
    for row_num, row_data in enumerate(filtered_data, start=start_row + 1):
        max_length = 0  # Initialize max length for text-based height adjustment
        line_breaks = 1  # Start with a base line count (1 line)
        ...
        # Adjust row height dynamically based on text length and line breaks
        # Base height of 15, increased by 0.5 for each 50 characters and each line break
        worksheet.row_dimensions[row_num].height = max(15, 15 + (max_length // 50) * 10 + line_breaks * 10)
