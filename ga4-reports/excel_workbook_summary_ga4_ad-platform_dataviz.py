...
# Notes: Third column of charts - WoW comparison according to specified groupings
...
vba_code_chart3 = """
Sub CreateThirdColumnCharts(metricName As String, chartNumber As Integer, leftPosition As Double, topPosition As Double, chartWidth As Double, chartHeight As Double)
    Dim ws As Worksheet
    Dim chartSheet As Worksheet
    Dim chartObj As ChartObject
    Dim myChart As Chart
    ...
    Select Case chartNumber
        Case 1
            ' Update Campaign_Group for both years
            For i = 3 To ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
                If ws.Cells(i, 1).Value = metricName And _
                   ((ws.Cells(i, 2).Value = 2023 And ws.Cells(i, 4).Value = "Non-Paid Total") Or _
                    (ws.Cells(i, 2).Value = 2024 And ws.Cells(i, 4).Value = "Non-Paid Total")) Then
                    If filteredRange Is Nothing Then
                        Set filteredRange = ws.Rows(i)
                    Else
                        Set filteredRange = Union(filteredRange, ws.Rows(i))
            ...
            Next i
        Case 2
            ' Similar logic for Case 2 with the updated Campaign_Group for both years
            For i = 3 To ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
                If ws.Cells(i, 1).Value = metricName And _
                   ((ws.Cells(i, 2).Value = 2023 And ws.Cells(i, 4).Value = "Paid Total") Or _
                    (ws.Cells(i, 2).Value = 2024 And ws.Cells(i, 4).Value = "Paid Total")) Then
                    If filteredRange Is Nothing Then
                        Set filteredRange = ws.Rows(i)
                    Else
        ...
        Case 3
            ...
            For i = 3 To ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
                If ws.Cells(i, 1).Value = metricName And _
                   ((ws.Cells(i, 2).Value = 2023 And ws.Cells(i, 4).Value = "SEM Brand Total") Or _
                    (ws.Cells(i, 2).Value = 2024 And ws.Cells(i, 4).Value = "SEM Brand Total")) Then
                    If filteredRange Is Nothing Then
                        Set filteredRange = ws.Rows(i)
                    Else
                        Set filteredRange = Union(filteredRange, ws.Rows(i))
        ...
        Case 4
            ...
            For i = 3 To ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
                If ws.Cells(i, 1).Value = metricName And _
                   ((ws.Cells(i, 2).Value = 2023 And ws.Cells(i, 4).Value = "SEM Non-Brand Total") Or _
                    (ws.Cells(i, 2).Value = 2024 And ws.Cells(i, 4).Value = "SEM Non-Brand Total")) Then
                    If filteredRange Is Nothing Then
                        Set filteredRange = ws.Rows(i)
                    Else
                        Set filteredRange = Union(filteredRange, ws.Rows(i))
        ...
        Case 5
            ...
            For i = 3 To ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
                If ws.Cells(i, 1).Value = metricName And _
                   ((ws.Cells(i, 2).Value = 2023 And ws.Cells(i, 4).Value = "Paid Social + Display Total") Or _
                    (ws.Cells(i, 2).Value = 2024 And ws.Cells(i, 4).Value = "Paid Social + Display Total")) Then
                    If filteredRange Is Nothing Then
                        Set filteredRange = ws.Rows(i)
                    Else
                        Set filteredRange = Union(filteredRange, ws.Rows(i))
        ...
        Case 6
            ...
            For i = 3 To ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
                If ws.Cells(i, 1).Value = metricName And _
                   ((ws.Cells(i, 2).Value = 2023 And ws.Cells(i, 4).Value = "Direct") Or _
                    (ws.Cells(i, 2).Value = 2024 And ws.Cells(i, 4).Value = "Direct")) Then
                    If filteredRange Is Nothing Then
                        Set filteredRange = ws.Rows(i)
                    Else
        ...
        Case 7
            ...
            For i = 3 To ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
                If ws.Cells(i, 1).Value = metricName And _
                   ((ws.Cells(i, 2).Value = 2023 And ws.Cells(i, 4).Value = "Organic Search (SEO)") Or _
                    (ws.Cells(i, 2).Value = 2024 And ws.Cells(i, 4).Value = "Organic Search (SEO)")) Then
                    If filteredRange Is Nothing Then
                        Set filteredRange = ws.Rows(i)
                    Else
        ...
        Case 8
            ...
            For i = 3 To ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
                If ws.Cells(i, 1).Value = metricName And _
                   ((ws.Cells(i, 2).Value = 2023 And ws.Cells(i, 4).Value = "Google SEM Total") Or _
                    (ws.Cells(i, 2).Value = 2024 And ws.Cells(i, 4).Value = "Google SEM Total")) Then
                    If filteredRange Is Nothing Then
                        Set filteredRange = ws.Rows(i)
                    Else
                        Set filteredRange = Union(filteredRange, ws.Rows(i))
        ...
        Case 9
            ...
            For i = 3 To ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
                If ws.Cells(i, 1).Value = metricName And _
                   ((ws.Cells(i, 2).Value = 2023 And ws.Cells(i, 4).Value = "Bing SEM Total") Or _
                    (ws.Cells(i, 2).Value = 2024 And ws.Cells(i, 4).Value = "Bing SEM Total")) Then
                    If filteredRange Is Nothing Then
                        Set filteredRange = ws.Rows(i)
                    Else
                        Set filteredRange = Union(filteredRange, ws.Rows(i))
        ...
        Case 10
            ...
            For i = 3 To ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
                If ws.Cells(i, 1).Value = metricName And _
                   ((ws.Cells(i, 2).Value = 2023 And ws.Cells(i, 4).Value = "Google Search - TNH Non-Brand") Or _
                    (ws.Cells(i, 2).Value = 2024 And ws.Cells(i, 4).Value = "Google Search - TNH Non-Brand")) Then
                    If filteredRange Is Nothing Then
                        Set filteredRange = ws.Rows(i)
                    Else
    ...
    End If
     ...
     ' Create the chart on the "WTD_CHART" worksheet at the provided top and left positions
    Set chartObj = chartSheet.ChartObjects.Add(Left:=leftPosition, Width:=chartWidth, Top:=topPosition, Height:=chartHeight)
    chartObj.Name = "Combination Chart_" & metricName & "_" & chartNumber
    Set myChart = chartObj.Chart
    ...
    ' Remove X-Axis title but keep tick labels at low position
    With myChart.Axes(xlCategory)
        .TickLabelPosition = xlLow ' Set X-Axis label position to Low
    End With
...
# Insert the VBA code for third column charts
try:
    new_module_chart3 = final_workbook.VBProject.VBComponents.Add(1)  # Add a new standard module
    new_module_chart3.Name = "ThirdColumnChartsModule3"
    new_module_chart3.CodeModule.AddFromString(vba_code_chart3)
    print("VBA code for third column charts successfully added to the final report.")
except Exception as e:
    print(f"Error while adding second column VBA code: {e}")
...
# Define the metrics and their positions for the third column charts
metrics = ["FF_Purchase_Event_Count_WoW", "FF_Lead_Event_Count_WoW"]
chart_left_position = 1850  # Adjust position for the third column
chart_top_position = 20  # Starting position for the third column charts
chart3_height = 200  # Set height for the smaller charts
chart3_width = 800  # Adjusted width for the third column charts
spacing_between_charts = 20  # Space between charts

# Run third column charts
try:
    for metric in metrics:
        for chart_num, top_position in enumerate([chart_top_position + i * (chart3_height + spacing_between_charts) for i in range(10)], start=1):
            macro_name = f"{final_workbook.Name}!ThirdColumnChartsModule3.CreateThirdColumnCharts"
            excel_app.Application.Run(macro_name, metric, chart_num, chart_left_position, top_position, chart3_width, chart3_height)  # Pass width and height
            print(f"Chart for {metric} (Case {chart_num}) created successfully.")
        # Adjust the top position for the next metric
        chart_top_position += 10 * (chart3_height + spacing_between_charts)  # Move the starting position down for the next set of charts
except Exception as e:
    print(f"Error while creating third column chart: {e}")
