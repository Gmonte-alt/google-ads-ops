...
# Notes: Creates Chart visualizations on the pivot_sheet - the first chart and formatting we want
...
Sub CreateLandlordCharts()
    ...
    ' Set the metric name to look for
    metric = "FF_Purchase_Event_Count"  ' You can replace this with a dynamic reference if needed
    ...
    Else
        ...
        MsgBox "Metric '" & metric & "' found in column " & rangeStartCol
    End If

    ' Define the start row (5 rows below where the metric name was found)
    startRow = foundCell.Row + 5

    ' Define number of rows (we'll use 52 rows as standard)
    numRows = 52
    ...
    ' Insert section name "Landlord Performance" in row 10, column E
    ws.Cells(10, 5).Value = "Landlord Performance"
    ws.Cells(10, 5).Font.Size = 20

    ' Create a chart in column E (start at row 13)
    chartTopPosition = ws.Cells(13, 5).Top  ' Column E
    Set chart = ws.ChartObjects.Add(Left:=ws.Cells(13, 5).Left, Top:=chartTopPosition, Width:=600, Height:=300)  ' Adjust position and size
    ...
    With chart.Chart.SeriesCollection(1)
        ...
        .Name = "=" & ws.Cells(startRow, rangeStartCol + 1).Address(External:=True) ' Series name is now dynamically linked to the cell ' seriesName1  ' Use the name from the first row of the range
        .ChartType = xlLine
        .AxisGroup = xlPrimary
    End With
    ...
    With chart.Chart.SeriesCollection(2)
        ...
        .Name = "=" & ws.Cells(startRow, rangeStartCol + 1).Address(External:=True) ' Series name is now dynamically linked to the cell ' seriesName2  ' Use the name from the first row of the range
        .ChartType = xlLine
        .AxisGroup = xlPrimary
    End With
    ...
    ws.Cells(12, 5).Formula = chartPrefix

    ' Set the chart title to reference the concatenation result in row 12
    chart.Chart.HasTitle = True
    chart.Chart.ChartTitle.Formula = "=" & ws.Cells(12, 5).Address(External:=True)
