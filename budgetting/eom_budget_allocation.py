...
# Adjust process_input_csv function to account for a reference date that's two days back from today
def process_input_csv(df_campaign_details):
    ...
    # Define the reference date as two days back from today
    reference_date = datetime.today() - timedelta(days=2)
    ...
    for time_frame, days in time_frames.items():
        # Adjust the end_date to be the reference date (two days back from today)
...
if __name__ == '__main__':
    ...
    # Condense the df_campaign_details to four columns and aggregate
    df_campaign_details = df_campaign_details[['Channel_Group', 'Campaign_Group', 'budget', 'recommended_budget_amount']]

    # Aggregate the budget and recommended_budget_amount columns by Channel_Group and Campaign_Group
    df_campaign_details = df_campaign_details.groupby(['Channel_Group', 'Campaign_Group'], as_index=False).agg({
        'budget': 'sum',
        'recommended_budget_amount': 'sum'
    })
